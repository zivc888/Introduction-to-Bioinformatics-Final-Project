...
meta3 <- read.delim("../Databases/Bipolar - E-GEOD-5388/E-GEOD-5388-experiment-design.tsv")
...
dds <- DESeqDataSetFromMatrix(countData=counts, colData=meta1, design= ~Diagnosis, tidy=TRUE)


row.names(counts) <- counts$Gene.ID
meta1$Diagnosis <- factor(meta1$Diagnosis)

design <- model.matrix(~meta1$Diagnosis)
...
EnhancedVolcano(tt,lab = rownames(tt),x = "logFC", y = "adj.P.Val", pCutoff = 0.05, FCcutoff = 0.1)
