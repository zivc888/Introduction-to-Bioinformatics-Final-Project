...
i <- 1
for (a1 in areas){
  for (a2 in areas[areas > a1]){
    comparison <- paste0(a1, " vs ", a2)
    print(paste0("Calculating ", comparison))
    resLFC.areas[[i]] <- lfcShrink(dds[dds$Brodmann %in% c(a1, a2)], contrast = c("Diagnosis", "bipolar", "normal"), type = "ashr")
...
genes <- c("LINC02340", "MTND6P4", "MT1X")
...
for (gene in genes) {
  ...
  print(
    ggplot(plot[plot$count < cutoffs[which(genes == gene)],], aes(x = Diagnosis, y = count, fill=Diagnosis)) + 
      ...
      geom_jitter(color="royalblue", size=0.6, alpha=0.5) + 
      theme(legend.position="none")  + 
      ggtitle(paste0("Cut Gene Expression of ", gene))
    )
}
```
