...
for (gene in genes) {
  ...
  print(
    ggplot(plot[plot$count < cutoffs[which(genes == gene)],], aes(x = Diagnosis, y = count, fill=Diagnosis)) + 
      geom_violin(draw_quantiles = 0.5) + 
      geom_jitter(color="royalblue", size=0.6, alpha=0.5) + 
      theme(legend.position="none")  + 
      ggtitle(paste0("Cut Gene Expression of ", gene, " p=", resLFC.bipolar.normal$padj[gene.index]))
    )
}
```
