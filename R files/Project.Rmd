...
for (a1 in areas){
  for (a2 in areas[areas > a1]){
    ...
    resLFC.areas[[i]] <- lfcShrink(dds, contrast = c("Brodmann", a1, a2), type = "ashr")
...
for (i in seq_along(comparisons)){
  print(EnhancedVolcano(resOrdered.areas[[i]], lab = resOrdered.areas[[i]]$symbol, x = 'log2FoldChange', y = 'padj', FCcutoff=2, pCutoff = 10e-10, title=comparisons))
}
...
colnames(normcounts) <- paste0(metadata$Diagnosis, seq(1, length(metadata$Diagnosis)))
var_per_gene <- apply(normcounts, 1, var)
selectedGenes <- names(var_per_gene[order(var_per_gene, decreasing = T)][1:1000])
normcounts.top1Kvar <- normcounts[selectedGenes,]
